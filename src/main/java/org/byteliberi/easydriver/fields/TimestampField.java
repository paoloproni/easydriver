...
/**
 * This is a table field that matches the TIMESTAMP database type.
 * 
 * @author Paolo Proni
 * @since 1.0
 * @version 1.0
 */
public class TimestampField extends TableField<Date> {	
	/**
	 * This instance is used when the caller needs the map methods only.
	 */
	private final static TimestampField empty = new TimestampField();
	 ...
	 */
	public static TimestampField getEmpty() {
		return empty;
	}
	 ...
	 */
	public TimestampField(final String name, final DBTable<?> table) {
		super(name, table);
	}
	 ...
	 */
	public TimestampField(final String name, final boolean annullable, final DBTable<?> table) {
		super(name, annullable, table);
	}

	@Override
	public Date map(final ResultSet rs, final int index) throws SQLException {
		final Date res = rs.getTimestamp(index);
		if (rs.wasNull())
			return null;
		else
			return res;
	...
	public void map(final PreparedStatement pstm, final int index, final Date value) throws SQLException {     
		if (value == null) {
			if (isAnnullable())
				pstm.setNull(index, Types.TIMESTAMP);
			else
				throw new SQLException("Attempt to set a null value to the field " + getCompleteName());
		}
		else
			pstm.setTimestamp(index, new Timestamp(value.getTime()));
	}
}
