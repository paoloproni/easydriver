...
/**
 * This is a table field that matches the DOUBLE database type.
 * 
 * @author Paolo Proni
 * @since 1.0
 * @version 1.0
 */
public class DoubleField extends TableField<Double> {
	/**
	 * This instance is used when the caller needs the map methods only.
	 */
	private final static DoubleField empty = new DoubleField();
	 ...
	 */
	public static DoubleField getEmpty() {
		return empty;
	}
	 ...
	 */
	public DoubleField(final String name, final DBTable<?> table) {
		super(name, table);
	}
	 ...
	 */
	public DoubleField(final String name, final boolean annullable, final DBTable<?> table) {
		super(name, annullable, table);
	}
	
	@Override
	public Double map(final ResultSet rs, final int index) throws SQLException {
		final double res = rs.getDouble(index);
		if (rs.wasNull())
			return null;
		else
			return res;
	...
	@Override
	public void map(final PreparedStatement pstm, final int index, final Double value) throws SQLException {
		if (value == null) {
			if (isAnnullable())
